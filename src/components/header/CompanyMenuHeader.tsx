...
export interface ICompanyMenuHeaderProps {
  darkMode?: boolean;
}

const CompanyMenuHeader = (props: ICompanyMenuHeaderProps) => {
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div className={` text-darkGrey uppercase cursor-pointer`}>
            {"Our Company"}
          </div>
          <div className={` text-darkGrey uppercase cursor-pointer`}>
            {"Locations"}
      ...
      {isMenuOpen && (
        <nav className={"bg-[rgba(0,0,0,0.5)] h-[calc(100vh-96px)] sm:hidden"}>
          <div className={"px-6 py-12 grid bg-black gap-8"}>
            <div
              className={` bg-black text-white text-[1.5rem] leading-[25px]  uppercase cursor-pointer`}
            >
              {"Our company"}
            </div>
            <div
...
export default CompanyMenuHeader;
